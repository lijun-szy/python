...
import time
# 定义图表上方大标题
title = "Server network  traffic flow ("+time.strftime(
...
rrdtool.graph("Flow.png", "--start", "-1d", "--vertical-label=Bytes/s", "--x-grid", "MINUTE:12:HOUR:1:HOUR:1:0:%H",
              ...
              "DEF:inoctets=Flow.rrd:enp0s3_in:AVERAGE",  # 指定网卡入流量数据源DS及CF
              "DEF:outoctets=Flow.rrd:enp0s3_out:AVERAGE",  # 指定网卡出流量数据源DS及CF
